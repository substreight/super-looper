...
def _declared_verifier_status(manifest: Dict[str, Any], repo_path: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "path_confirmed": bool(candidate_paths) and not missing_paths,
...
def _python_ast_corpus_test(threshold: float = 0.10, sample_limit: int = 40) -> str:
    """Return a pytest sketch verifier for Headroom-style Python AST compression."""
    return f'''"""Sketch verifier for Python AST compression syntax validity.
...
def test_python_ast_compression_corpus_preserves_syntax(caplog):
    ...
    assert corpus, "sketch verifier found no Python corpus files with functions/classes"
    ...
    assert successful_compressions > 0, "sketch verifier produced no smaller valid compressed outputs"
...
def _sketch_template_files(template: str, manifest: Dict[str, Any]) -> Dict[str, str]:
    if template != "python-ast-corpus":
        raise CaseStudyError(f"unsupported sketch verifier template: {template}")
    raw = manifest.get("sketch_verifier")
    if not isinstance(raw, dict):
        raw = manifest.get("shadow_verifier")  # back-compat: accept the old manifest key for one release
    config = raw if isinstance(raw, dict) else {}
    threshold = config.get("invalid_rate_threshold", 0.10)
    sample_limit = config.get("sample_limit", 40)
    if not isinstance(threshold, (int, float)) or threshold < 0 or threshold > 1:
        raise CaseStudyError("sketch_verifier.invalid_rate_threshold must be a number from 0 to 1")
    if not isinstance(sample_limit, int) or sample_limit < 1:
        raise CaseStudyError("sketch_verifier.sample_limit must be a positive integer")
...
def simulate_sketch_verifier(
    ...
    This intentionally produces sketch evidence only. Passing here means the proposed
    ...
    _json_write(os.path.join(run_dir, "issue.json"), {
        "source": manifest.get("issue") or "",
        "note": "Sketch verifier simulation; issue context is not fetched by the local harness.",
    })

    proposed_files = _sketch_template_files(template, manifest)
    proposed_root = os.path.join(run_dir, "sketch-proposed")
    ...
    _text_write(os.path.join(run_dir, "sketch.patch"), patch_text)
    ...
    if scope.get("passed"):
        command_results.append(_run_command(
            ...
            repo_path,
            timeout_seconds,
            os.path.join(run_dir, "sketch-command-1.stdout.txt"),
            os.path.join(run_dir, "sketch-command-1.stderr.txt"),
        ))
    verifier_results = {
        "skipped": False,
    ...
    sketch = {
        ...
        "patch": "sketch.patch",
        "proposed_root": "sketch-proposed",
        "scope_passed": scope.get("passed") is True,
        "verifier_passed": verifier_results["passed"],
        "status": "sketch_verified" if verifier_results["passed"] and scope.get("passed") else "sketch_failed",
        "note": "Sketch verification runs proposed verifier code from artifacts. It is not upstream or CI evidence.",
    }
    _json_write(os.path.join(run_dir, "sketch-verifier.json"), sketch)

    summary = summarize_run(run_dir)
    _json_write(os.path.join(run_dir, "summary.json"), summary)
    write_reports(run_dir)
    return {"run_dir": run_dir, "summary": summary, "sketch_verifier": sketch}


# Back-compat alias: a "verifier sketch" was previously called a "shadow verifier".
simulate_shadow_verifier = simulate_sketch_verifier
...
def resolve_verifier(
    ...
    missing, generate a sketch verifier by default. With sketch disabled, write a
    ...
    resolution = {
        "sketch_enabled": bool(sketch),
        **status,
    }

    if status["path_confirmed"]:
        resolution.update({
    ...
    if status["has_declared_verifier"] and not status["candidate_paths"]:
        # A real command with no file-path token (e.g. `make check`): run it, but it is
        # not statically confirmable as a gate, so its evidence can never reach a PR claim.
        resolution.update({
            "status": "declared_verifier_unconfirmed",
            "evidence_level": "unconfirmed",
            "action": "run_unconfirmed_verifier",
        })
        result = run_case_study(manifest_path, repo_path, run_id=run_id)
        result["summary"] = _write_resolution(result["run_dir"], resolution)
        result["verifier_resolution"] = resolution
    ...
    if sketch:
        resolution.update({
            "status": "declared_verifier_missing_sketch_attempted",
            "evidence_level": "sketch",
            "action": "simulate_sketch_verifier",
        })
        result = simulate_sketch_verifier(manifest_path, repo_path, template=template, run_id=run_id)
        result["summary"] = _write_resolution(result["run_dir"], resolution)
        result["verifier_resolution"] = resolution
        return result

    resolution.update({
        "status": "declared_verifier_missing_sketch_disabled",
...
def summarize_run(run_dir: str) -> Dict[str, Any]:
    ...
    sketch = _load_optional_json(os.path.join(run_dir, "sketch-verifier.json"))
    ...
    is_sketch = bool(sketch)
    res_level = resolution.get("evidence_level")
    if res_level in ("confirmed_local", "unconfirmed", "sketch", "missing"):
    ...
    claim_allowed = {
        ...
        "unconfirmed": "declared_unverified",
        "sketch": "proposal_only",
        "missing": "none",
    }[evidence_level]
    ...
    return {
        ...
        "sketch_proposed_files": sketch.get("proposed_files", []),
        ...
        "ready_for_sketch_report": is_sketch and verifier_passed and scope_passed and not errors,
...
def verify_run(run_dir: str) -> Dict[str, Any]:
    ...
    if summary.get("evidence_level") == "sketch":
        failures.append("only sketch verifier evidence is available")
...
def render_report(run_dir: str, audience: str = "maintainer") -> str:
    ...
    if resolution:
        lines.extend([
            ...
            f"- Sketch enabled: {'yes' if resolution.get('sketch_enabled') else 'no'}",
        ])
    ...
    if sketch:
        lines.extend([
            "## Sketch Verifier",
            "",
            f"- Status: {sketch.get('status') or 'unknown'}",
            f"- Template: {sketch.get('template') or 'unknown'}",
            f"- Patch: `{sketch.get('patch') or 'sketch.patch'}`",
            "- Proposed files:",
        ])
        for path in sketch.get("proposed_files") or []:
            lines.append(f"  - `{path}`")
        lines.extend([
            "",
    ...
    if sketch:
        artifacts.extend(["sketch-verifier.json", "sketch.patch", "sketch-proposed/"])
    ...
    if summary.get("evidence_level") == "sketch" and summary.get("ready_for_sketch_report"):
        lines.extend([
            "",
            "## Status",
            "",
            "This sketch verifier passed. Treat it as a proposed test patch, not as upstream verification.",
        ])
        ...
        lines.extend([
            ...
            "## Status",
            "",
            "No confirmed verifier was available. Enable sketch verifiers or add/promote a real repo-local gate before making a verification claim.",
        ])
